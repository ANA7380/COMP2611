...
updateBackgroundFloor:
    ...
    # step 3: check if backgroundBox.x + backgroundBox.width is <= 0,
    #         if true, set backgroundBox.x to 0.
    lw $s0, 0($t7) # s0 = backgroundBox.x
    lw $s1, 8($t7) # s1 = backgroundbox.width
    add $t0, $s0, $s1
    bgtz $t0, t2step4
    ...
    #         if true, set floorBox.x to floorBox.x + floorBox.width
t2step4:
    lw $s0, 0($t6) #s0 = floorbox.x
    lw $s1, 8($t6) #s1 = flooxbox.width
    add $t0, $s0, $s1 #t0 = floorbox.x + flooxbox.width
    bgtz $t0, t2return
...
# each box is a Rectangle
#--------------------------------------------------------------------
    #First Check: rectangle1.x < rectangle2.x + rectangle2.width
    #$a0: rectangle1 address, $a1: rectangle2 address
    lw $t0, 0($a0) #t0 = Rec1.x
    lw $t1, 0($a1) 
    lw $t2, 8($a1) #t2 = Rec2.width
    add $t1, $t1, $t2 #t1 = Rec2.x + rec2.width
    bge $t0, $t1, setfalse
    #Second check: rectangle1.x + rectangle1.width > rectangle2.x
    lw $t0, 0($a0) #t0 = Rec1.x
    lw $t1, 8($a0)
    add $t1, $t0, $t1 #t1 = rec1.x + rec1.width
    lw $t2, 0($a1) #t2 = Rec2.x
    ble $t1, $t2, setfalse
    #Third check: rectangle1.y < rectangle2.y + rectangle2.height
    lw $t0, 4($a0) #t0 = rec1.y
    lw $t1, 4($a1) #t1 = rec2.y
    lw $t2, 12($a1) #t2 = rec2.height
    add $t1, $t1, $t2 #t1 = rec2.y + rec2.height
    bge $t0, $t1, setfalse
    #Fourth check:rectangle1.y + rectangle1.height > rectangle2.y
    lw $t0, 4($a0) #t0 = rec1.y
    lw $t1, 12($a0) # t1 = rec1.height
    add $t1, $t0, $t1 #t1 = rec1.y + rec1.height
    lw $t2, 4($a1) #t2 = rec2.y
    ble $t1, $t2, setfalse
